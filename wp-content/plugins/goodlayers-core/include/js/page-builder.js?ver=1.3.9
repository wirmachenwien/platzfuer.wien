...
(function($){
  ...
  // https://stackoverflow.com/q/2901102/5157195
	function format_numberWithThousandSep(x) {
    x = x.toString();
    var pattern = /(-?\d+)(\d{3})/;
				...
				$({ counter_num: start_num }).animate({ counter_num: end_num }, { duration: duration_time, easing: 'easeOutExpo', 
					step: function(){
						if( this.counter_num > 0.3 ){
							counter_item.html( format_numberWithThousandSep(Math.ceil(this.counter_num)) );
						}else{
							counter_item.html( 0 );
						}
